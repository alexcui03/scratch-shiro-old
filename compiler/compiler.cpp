...
std::string compiler::compile_project(const std::string &src) {
    ...
    this->main_code = "ccvm::runtime runtime;" ENDL;
    this->var_map.clear();
...
void compiler::compile() {
    ...
    this->code << "int main() {" ENDL << this->main_code
...
void compiler::compile_target(scratch_target *target, int i) {
    ...
    // init target
    if (target->is_stage) {
        this->main_code << class_name << " " << class_name << "(&runtime);" ENDL;
    }
    else {
        this->main_code << class_name << " " << class_name << "(&runtime, &target_0);" ENDL;
    }
    ...
    for (int j = 0; auto block : target->blocks) {
        ...
        if (block->opcode == "procedures_definition") {
            this->code << "ccvm::coroutine " << func_name << "(";
            ...
            for (auto arg : prototype_src->inputs) {
                ...
                this->code << "const ccvm::variant &arg_" << std::to_string(prototype.arg_name_id.size());
            ...
            if (block->opcode == "event_whenflagclicked") {
                this->main_code << "runtime.push_thread(new ccvm::thread(std::bind(&"
