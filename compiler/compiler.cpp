...
std::string compiler::compile_project(const std::string &src) {
    ...
    this->code = "// This is generated by shiro-compiler.\n"
        "#include <vm/vm.h>\n\n";
    this->main_code = "shiro::runtime *runtime = new shiro::runtime;" ENDL;
    this->var_map.clear();
...
void compiler::compile() {
    ...
    this->code << "shiro::runtime *scratch_init() {" ENDL << this->main_code
...
void compiler::compile_target(scratch_target *target, int i) {
    ...
    this->code << "class " << class_name << ": public shiro::"
    ...
    for (int j = 0; auto var : target->variables) {
        this->code << "shiro::variant var_" << std::to_string(j) << ";" ENDL;
    ...
    // constructor
    if (target->is_stage) {
        this->code << class_name << "(shiro::runtime *rt): shiro::" << parent_class << "(rt) {" ENDL;
    }
    else {
        this->code << class_name << "(shiro::runtime *rt, target_0 *t): shiro::" << parent_class << "(rt), stage(t) {" ENDL;
        this->code << "runtime->add_target(this, \"" << target->name << "\");" ENDL;
    }
    // load all costumes
    for (auto &costume : target->costumes) {
        if (costume->bitmap_resolution != -1) {
            this->code << "this->load_costume(\"" << costume->name << "\", \"assets/" << costume->md5ext << "\", " << std::to_string(costume->bitmap_resolution) << ");" ENDL;
        }
        else {
    ...
    for (int j = 0; auto block : target->blocks) {
        ...
        if (block->opcode == "procedures_definition") {
            this->code << "shiro::coroutine " << func_name << "(";
            ...
            for (auto arg : prototype_src->inputs) {
                ...
                this->code << "const shiro::variant &arg_" << std::to_string(prototype.arg_name_id.size());
            ...
            this->code << "shiro::coroutine " << func_name << "() {" ENDL;
            // maybe we should move below codes to an event_map for generating codes.
            // @todo
            if (block->opcode == "event_whenflagclicked") {
                this->main_code << "runtime->push_thread(new shiro::thread(std::bind(&"
...
scratch_target *parse_target(const Json::Value &root) {
    ...
    for (auto costume_src : root["costumes"]) {
        auto costume = new scratch_costume{
            ...
            .rotation_center_x = costume_src["rotationCenterX"].asInt(),
            .rotation_center_y = costume_src["rotationCenterY"].asInt()
        };
        if (costume_src["bitmapResolution"]) {
            costume->bitmap_resolution = costume_src["bitmapResolution"].asInt();
        }
        else {
            costume->bitmap_resolution = -1;
        }
